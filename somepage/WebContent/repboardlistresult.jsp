...
$(function(){
    ...
    $("#forwordBt").click(function(){
      ...
      if(selectValue== "reply"){
        ...
        var parentNo = $("div.detail").find(".no").html();
        var parentsubject = $("div.detail").find(".subject").html();
        //★★리플달기 화면은 비번확인 필요없음 변경  
        
         var replyParent = $("#replyParent");
         var table = $("<table>").appendTo(replyParent);                
         var tr1 = $("<tr>").appendTo(table);
         var tr2 = $("<tr>").appendTo(table);
         var td = $("<td>").appendTo(tr1);
         var td1 = $("<td>").appendTo(tr2);
         var td2 = $("<td>").appendTo(tr2);
         tr1.html();
         tr2.html();
         td.html("수정할 글의 정보")
         td1.html(parentNo);
         td2.html(parentsubject);
         td1.css({"width": "50px"});  
        
      		 //$("div.detail").find(".no").html("답글쓸 번호:"+subjectValue);
        ...
        $.ajax({
          ...
          data:{'no': $no.trim()}, 
             ...
             for(var i=0; i<1; i++){
               ...
               $divDetail.find(".filename").html(jsonObj[i].filename);
             ...
             //답글 보여주기
              for(var j=1; j<listLen; j++){
               ...
               /* <h5 class="noInfo"> ${no} 번글에 대한 답글 목록 </h5> */
               var noInfo=$("#noInfo");
               //noInfo.val(html(jsonObj[j].no)+"번글에 대한 답글");
               var h5 = $("<h5>").appendTo(noInfo);
               h5.val($divDetail.find(".no").val());
  ...
  <table >
    ...
    <tr>
     ...
     <td style = "text-align:left">
       <c:forEach var="level" begin="2"  end="${repboard.level}">
       <img src = "reBt.png">
 ...
 <!-- 검색시 리스트 -->
  ...
  <%--  <table >
    ...
    <tr>
     ...
     <!-- 페이징  --> 
끝점 설정중
   <div class="paging" style="display: inline-block;  margin: 10px   0px    0px  50px;">
   ...
   <c:choose>
     ...
     <c:when test="${list_size%10==0}">
       필요한 페이지수 : ${list_size/10} 
       
       <c:if test="${list_size>6}">
       <c:forEach var="page" begin="${requestScope.start/10+1}" end="${requestScope.start/10+6}">
       [<a href = "">${page}</a>]
       </c:forEach> 
       ...
       <c:if test="${list_size<=6}">
       <c:forEach var="page" begin="${requestScope.start/10+1}" end="${requestScope.start/10+1+6}">
       [<a href = "">${page}</a>]
       </c:forEach>
       </c:if>
   ...
   <c:when test="${list_size%10!=0}">
       ...
       <c:if test="${list_size<=6}">
       <c:forEach var="page" begin="${requestScope.start/10+1}" end="${requestScope.start/10+7}">
       [<a href = "">${page}</a>]
       </c:forEach>
       </c:if>
  ...
  <%--  <c:otherwise test="${list_size>10}">
   <c:forEach var="page" begin="1" end="${pageNum+1}">
   [<a href = "">${page}</a>]<a href = "">▶</a>
   </c:forEach> 
   </c:otherwise> --%>
   </c:choose>
   </div>
...
<div class="detail">
  ...
  <tr>
  <td  class="filename" colspan="2" style="height : 10px ; padding : 0px ;">~</td>
  </tr>
  <tr>
  ...
  <%-- <c:set var="no" value="${requestScope.replyNo}"/> --%>
...
<div class="chkpassword" style="position: absolute; 
  ...
  <form class="passwordsubmit" method="post">
